...
(function () {
    ...
    function goTo(index) {
        ...
        // Congela a escala no ponto atual e envia para o topo (z-index 3) dissolvendo
        ...
        // Garante que o slide entrante não carregue estado anterior
        ...
        // Remove a classe leaving após o fade-out concluir
        setTimeout(function () {
...
function initClientsTicker() {
    ...
    // Cria tooltip fixo no body (escapa de qualquer overflow)
    const tooltip = document.createElement('div');
    tooltip.className = 'clients-tooltip';
    ...
    // Delegação de eventos no track — mouse
    track.addEventListener('mouseover', (e) => {
        const item = e.target.closest('.client-logo');
    ...
    // Delegação de eventos no track — touch (mobile)
    track.addEventListener('touchstart', (e) => {
        const item = e.target.closest('.client-logo');
        if (!item) return;
        e.preventDefault(); // evita scroll acidental e delay de 300ms
        currentItem = item;
        track.style.animationPlayState = 'paused';
        showTooltip(item);
    }, { passive: false });
